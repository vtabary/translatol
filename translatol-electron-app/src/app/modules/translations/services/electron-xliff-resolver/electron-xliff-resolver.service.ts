...
export class ElectronXliffResolverService implements XliffResolverService {
  ...
  resolve(route: ActivatedRouteSnapshot, _state: RouterStateSnapshot): Observable<ResolvedXLIFF> {
    const filePath = atob(route.params.properties);
    const templatePath = this.pathService.getTemplatePath(filePath);
    return combineLatest([this.fileService.open(filePath), this.fileService.open(templatePath)]).pipe(
