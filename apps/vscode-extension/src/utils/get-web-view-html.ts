...
export function getWebViewHTML(webview: Webview, extensionUri: Uri) {
  ...
  const angularCSSURI = webview.asWebviewUri(
    Uri.joinPath(extensionUri, 'webview', 'styles.css')
  );
  const angularRuntimeURI = webview.asWebviewUri(
    Uri.joinPath(extensionUri, 'webview', 'runtime.js')
  );
  const angularPolyfillsURI = webview.asWebviewUri(
    Uri.joinPath(extensionUri, 'webview', 'polyfills.js')
  );
