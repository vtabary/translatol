...
export class TranslationNavigationComponent {
  ...
  public ngOnChanges(changes: SimpleChanges): void {
    if (changes.toTranslate) {
      this.toTranslateTree = this.treeBuilder.build(this.toTranslate.map(item => item.$.id), '.');
    }

    if (changes.translated) {
      this.translatedTree = this.treeBuilder.build(this.translated.map(item => item.$.id), '.');
    }
  }
