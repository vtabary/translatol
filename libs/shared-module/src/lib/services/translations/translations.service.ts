...
export class TranslationsService {
  ...
  public filterByTranslatedOperator(
    translations: IXliffTransUnit[],
    translated = true
  ): IXliffTransUnit[] {
    return translations.filter(
      (translation) =>
        translation.children.some((child) => child.name === 'target') ===
        translated
    );
  }
  ...
  private getDuplicated(
    currentTranslations: IXliffTransUnit[]
  ): IXliffTransUnit[] {
    const keys = new Set();
  ...
  private handleTranslationObsolete(
    currentTranslations: IXliffTransUnit[],
    templateTranslations: IXliffTransUnit[]
  ): void {
    this.$isObsoleteTranslation = currentTranslations.some(
      (currentTrad) =>
        templateTranslations.filter((trad) => trad.$.id === currentTrad.$.id)
          .length === 0
    );
  }
}
